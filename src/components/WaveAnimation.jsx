...
function WaveAnimation() {
  ...
  useEffect(() => {
    ...
    const camera = new THREE.PerspectiveCamera(75, currentMount.clientWidth / currentMount.clientHeight, 1, 10000);
    ...
    const animate = () => {
      ...
      for (let ix = 0; ix < AMOUNTX; ix++) {
        for (let iy = 0; iy < AMOUNTY; iy++) {
          ...
          particlePositions[i + 1] = (Math.sin((ix + count) * 0.3) * 50) + (Math.sin((iy + count) * 0.5) * 50) - 500;
          
          const waveScale = (Math.sin((ix + count) * 0.3) + 1) * 4 + (Math.sin((iy + count) * 0.5) + 1) * 4;

          const distSq = (particleX - intersectionPoint.x)**2 + (particleZ - intersectionPoint.z)**2;
          const color = new THREE.Color().fromArray(particleColors, i);
          
          // Check for ripple effect from clicks
          let rippleIntensity = 0;
          if (window.rippleEffect && window.rippleEffect.time < window.rippleEffect.maxTime) {
            const rippleDistSq = (particleX - window.rippleEffect.x)**2 + (particleZ - window.rippleEffect.z)**2;
            const rippleRadius = (window.rippleEffect.time / window.rippleEffect.maxTime) * 800;
            const rippleThickness = 100;
            
            if (Math.abs(Math.sqrt(rippleDistSq) - rippleRadius) < rippleThickness) {
              rippleIntensity = 1 - (window.rippleEffect.time / window.rippleEffect.maxTime);
            }
          }
          ...
          if (distSq < hoverRadius**2) {
             ...
             particleScales[particleIndex] = THREE.MathUtils.lerp(particleScales[particleIndex], waveScale, 0.1);
          }
